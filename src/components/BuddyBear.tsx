...
export function BuddyBear({ mood = 'idle', speech, size = 120, className = '', onClick }: Props) {
  ...
  return (
    <div 
      onClick={onClick}
      className={`flex items-center gap-2 ${className} ${onClick ? 'cursor-pointer active:scale-95 transition-transform' : ''}`}
    >
      <AnimatePresence>
        {speech && (
          <motion.div
