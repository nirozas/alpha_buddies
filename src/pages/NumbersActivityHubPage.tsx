...
export function NumbersActivityHubPage() {
  const navigate = useNavigate();

  useEffect(() => {
    document.body.classList.add('full-opacity-bg');
    return () => document.body.classList.remove('full-opacity-bg');
  }, []);

  const recognition = NUMBER_ACTIVITIES.filter(a => a.group === 'recognition');
  const formation = NUMBER_ACTIVITIES.filter(a => a.group === 'formation');
  const usage = NUMBER_ACTIVITIES.filter(a => a.group === 'usage');

  const renderGroup = (title: string, activities: typeof NUMBER_ACTIVITIES, isWide = false) => (
    <div className={`mb-4 w-full ${isWide ? 'max-w-6xl' : 'max-w-xl'} px-4`}>
      <div 
      ...
      <div className={`grid gap-3 ${isWide ? 'grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6' : 'grid-cols-2 sm:grid-cols-3 md:grid-cols-4'}`}>
        {activities.map((act) => (
          <ActivityCard
            key={act.id}
            name={act.name}
            ...
            icon={act.icon}
            onClick={() => {
              if (act.id === 'number-of-the-day') {
                navigate(`/numbers/activity/${act.id}/random`);
              } else {
                navigate(`/numbers/activity/${act.id}`);
              }
            }}
            smaller={true}
          />
    ...
    <>
      ...
      <div className="min-h-dvh flex flex-col items-center pb-24 relative z-10">
        <div className="w-full flex items-center p-4 sticky top-0 z-50">
          <BackButton onClick={() => navigate('/')} color="#2563EB" />
        ...
        <div className="absolute top-4 right-4 z-[60]">
          <BuddyBear mood="happy" size={80} speech="Pick a game to play, then we'll choose a number!" />
        </div>

        {/* Fluid Landscape Distribution */}
        <div className="w-full max-w-[1600px] px-8 flex flex-col gap-6">
          {/* Row 1: Left and Right Trees */}
          <div className="flex flex-col lg:flex-row justify-between items-start gap-12">
            <div className="w-full lg:w-auto">
              {renderGroup('1. Recognition', recognition)}
            </div>
            <div className="w-full lg:w-auto lg:text-left flex lg:justify-end">
