...
export function LetterSelectorPage() {
  ...
  const [lotd] = useState(() => LETTERS_DATA[Math.floor(Math.random() * LETTERS_DATA.length)]);

  return (
    <div className="min-h-dvh flex flex-col items-center pb-32">
      <div className="w-full flex items-center p-4 sticky top-0 bg-white/20 backdrop-blur-md z-10 border-b border-white/10">
      ...
      <motion.div 
        ...
        className="w-full max-w-4xl px-4 mt-4 mb-8"
      >
        <div 
          className="bg-white rounded-[2rem] p-6 shadow-xl border-4 flex flex-col md:flex-row items-center gap-6 relative overflow-hidden"
          style={{ borderColor: lotd.color }}
        >
          <div className="absolute top-0 right-0 w-32 h-32 opacity-5 pointer-events-none" style={{ backgroundColor: lotd.color, borderRadius: '0 0 0 100%' }} />
          
          <div className="flex flex-col items-center gap-2">
            <span className="text-sm font-black uppercase tracking-widest opacity-50" style={{ color: lotd.color }}>Letter of the Day</span>
            <motion.button
              whileHover={{ scale: 1.1, rotate: 5 }}
              whileTap={{ scale: 0.9 }}
              onClick={() => navigate(`/letters/activity/${activity}/${lotd.letter}`)}
              className="w-24 h-24 rounded-2xl flex items-center justify-center text-5xl font-black text-white shadow-lg"
              style={{ backgroundColor: lotd.color }}
            >
              {lotd.uppercase}
            </motion.button>
          </div>
          ...
          <div className="flex-1 text-center md:text-left">
            <h3 className="text-2xl font-black mb-1" style={{ color: lotd.color }}>Meet the letter {lotd.uppercase}!</h3>
          ...
          <button
            onClick={() => navigate(`/letters/activity/${activity}/${lotd.letter}`)}
            className="px-8 py-3 rounded-2xl font-black text-white shadow-lg transition-transform active:scale-95"
            style={{ backgroundColor: lotd.color }}
          >
            Play Now! 🚀
          </button>
      ...
      <div className="text-center px-4 mb-6 flex flex-col items-center gap-3">
        <p className="text-xl font-bold text-gray-700" style={{ fontFamily: 'Nunito' }}>
          {isMulti ? "Select multiple letters to play!" : "Or select any letter below!"}
        </p>
        {isMulti && (
          <button 
