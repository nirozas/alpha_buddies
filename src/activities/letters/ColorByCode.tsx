...
import { TEMPLATES, type ColorTemplate } from './colorByCodeTemplates';
...
export function ColorByCode() {
  ...
  // Setup state
  const [step, setStep] = useState<1 | 2 | 3>(1); // 1: picture, 2: letters, 3: game
  ...
  // Map colorIds to letter objects based on selection
  const colorMapping = useMemo(() => {
    const map: Record<number, LetterData> = {};
    const availableColorIds = Object.keys(selectedTemplate.palette).map(Number).sort();
    
    // Assign a color to each selected letter up to the max colors available
    selectedLetters.forEach((letterStr, idx) => {
      if (idx < availableColorIds.length) {
        const ld = LETTERS_DATA.find(l => l.letter === letterStr);
        if (ld) map[availableColorIds[idx]] = ld;
      }
    });
    return map;
  }, [selectedLetters, selectedTemplate]);
  ...
  const handleRegionClick = (regionId: string, regionColorId: number) => {
    ...
    const assignedLetter = colorMapping[regionColorId];
    if (!assignedLetter) {
    ...
    if (selectedColorId === regionColorId) {
      ...
      if (newFilled.length === targetRegions.length) {
        ...
        const lKey = selectedLetters.join('-');
      ...
      playBoing();
      setMistakes(m => m + 1);
      // Read the letter that belongs here
      let text = assignedLetter.uppercase;
      if (mode === 'small') text = assignedLetter.lowercase;
      speak(text);
    }
  };

  const handleCrayonClick = (colorId: number) => {
    const assignedLetter = colorMapping[colorId];
    if (assignedLetter) {
        ...
        <div className="max-w-5xl w-full grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {TEMPLATES.map(t => (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={() => { setSelectedTemplate(t); setSelectedLetters([]); setStep(2); playPop(); }}
      ...
      <div className="min-h-dvh bg-gradient-to-br from-green-50 to-blue-50 flex flex-col items-center p-8 relative z-10 font-nunito">
        ...
        <div className="max-w-4xl w-full grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-[2rem] p-6 shadow-xl border-4 border-slate-100 flex flex-col gap-6">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-700 mb-4">Letter Case</h2>
              <div className="flex gap-4">
                {(['capital', 'small', 'both'] as const).map(m => (
                  <button
                    key={m}
                    onClick={() => { setMode(m); playPop(); }}
                    className={`flex-1 py-3 rounded-xl font-bold capitalize transition-all ${mode === m ? 'bg-indigo-500 text-white shadow-lg' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                  >
                    {m}
                  </button>
                ))}
          ...
          <div className="bg-white rounded-[2rem] p-6 shadow-xl border-4 border-slate-100 flex flex-col">
            <h2 className="text-xl font-bold text-slate-700 mb-4 flex justify-between items-center">
              <span>Pick up to {Object.keys(selectedTemplate.palette).length} Letters</span>
              <span className="text-sm bg-slate-100 px-3 py-1 rounded-full">{selectedLetters.length} / {Object.keys(selectedTemplate.palette).length}</span>
            </h2>
            <div className="flex flex-wrap gap-2 overflow-y-auto max-h-64 pr-2">
              {LETTERS_DATA.map(ld => {
                ...
                const isDisabled = !isSelected && selectedLetters.length >= maxAllowed;
                ...
                return (
                  <motion.button
                    ...
                    onClick={() => {
                      if (isSelected) {
                        setSelectedLetters(selectedLetters.filter(l => l !== ld.letter));
                        playPop();
                      } else if (!isDisabled) {
                        setSelectedLetters([...selectedLetters, ld.letter]);
                        playPop();
                      } else {
                        playBoing();
                      }
            ...
            <motion.button
              whileHover={selectedLetters.length > 0 ? { scale: 1.02 } : {}}
              whileTap={selectedLetters.length > 0 ? { scale: 0.98 } : {}}
              onClick={handleStart}
              className={`mt-auto pt-6 w-full py-4 rounded-xl font-black text-xl uppercase tracking-widest transition-all ${selectedLetters.length > 0 ? 'bg-emerald-500 hover:bg-emerald-600 text-white shadow-xl shadow-emerald-500/20' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}
            >
              Start Coloring
            </motion.button>
          </div>
        ...
        <div className="bg-white p-4 rounded-[3rem] shadow-2xl border-8 border-slate-100 w-full max-w-[500px] aspect-square relative">
          <svg viewBox="0 0 400 400" className="w-full h-full rounded-[2rem] overflow-hidden" style={{ backgroundColor: '#F8FAFC' }}>
            ...
            {selectedTemplate.regions.map(region => {
              const assignedLetter = colorMapping[region.colorId];
              ...
              return (
                <motion.path
                  ...
                  className={assignedLetter && !autoFillRest ? "cursor-pointer" : ""}
                  d={region.d}
                  fill={fillHex}
                  stroke="#1E293B"
                  strokeWidth="4"
                  strokeLinejoin="round"
                  whileHover={assignedLetter && !isFilled && !autoFillRest ? { fill: '#F1F5F9' } : {}}
                  whileTap={assignedLetter && !isFilled && !autoFillRest ? { scale: 0.99 } : {}}
                />
              );
            })}
            ...
            {selectedTemplate.regions.map(region => {
              ...
              let char = '';
              if (assignedLetter && !isFilled) {
                if (mode === 'capital') char = assignedLetter.uppercase;
                else if (mode === 'small') char = assignedLetter.lowercase;
                else char = (region.id.length % 2 === 0) ? assignedLetter.uppercase : assignedLetter.lowercase;
              }
